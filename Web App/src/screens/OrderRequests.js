...
import { order_request } from '../ServiceLayer/Order';
...
class OrderRequests extends Component {
    ...
    handleSendToInProgressBtn(orderId) {
        var token = localStorage.getItem('securityToken');
        const options = {
            url: process.env.REACT_APP_NodeJS_ServiceURL + '/api/v1/orders/' + orderId,
            method: 'PUT',
            headers: {
    ...
    handleSendToDeliveredBtn(orderId) {
        var token = localStorage.getItem('securityToken');
        const options = {
            url: process.env.REACT_APP_NodeJS_ServiceURL + '/api/v1/orders/' + orderId,
            method: 'PUT',
                        ...
                        <div className="container border-bottom pb-2 px-lg-0 px-md-0 mb-4" key={orderRequest[val].id}>
                            <div className="row mb-3">
                                <div className="col-lg-6 col-md-6 col-12">
                                    <h5 className="">{orderRequest[val].user.userName}</h5>
                                </div>
                            ...
                            {
                                Object.keys(orderRequest[val].itemList).map((val2) => {
                                    // console.log(orderRequest[val].itemList[val2])
                                    // console.log(val2)
                                    return (
                                        <div className="row mb-3" key={val2}>
                                            <div className="col-lg-2 col-md-3 col-8 offset-2 offset-lg-0 offset-md-0 px-0 mb-3 text-center">
                                                <img style={{ width: "70px", height: "70px" }} alt="Natural Healthy Food" src={orderRequest[val].itemList[val2].itemImageUrl} />
                                            </div>
                                            <div className="col-lg-7 col-md-6 col-sm-12 px-0">
                                                <h6 className="">{orderRequest[val].itemList[val2].itemTitle}</h6>
                                                <p className="mb-1"><small>{orderRequest[val].itemList[val2].itemIngredients}</small></p>
                                            </div>
                                            <div className="col-lg-3 col-md-3 col-sm-12 px-0 text-right">
                                                <span style={{ fontSize: "14px" }} className="mx-3"><b>RS.{orderRequest[val].itemList[val2].itemPrice}</b></span>
                                            </div>
                                        </div>
                                    )
                                })
                            ...
                            <div className="row mb-3 mb-md-0 mb-lg-0">
                                <div className="col-lg-6 col-md-6 col-12 order-lg-first order-md-first order-last ">
                                    <button type="button" onClick={() => this.handleSendToInProgressBtn(orderId)} className="btn btn-warning btn-sm text-uppercase px-3"><b>Send To In Progress</b></button>
                                </div>
                ...
                if (orderRequest[val].status === "IN PROGRESS") {
                    return (
                        <div className="container border-bottom pb-2 px-lg-0 px-md-0 mb-4" key={orderRequest[val].id}>
                            ...
                            <div className="row mb-3 mb-md-0 mb-lg-0">
                                <div className="col-lg-6 col-md-6 col-12 order-lg-first order-md-first order-last ">
                                    <button type="button" onClick={() => { this.handleSendToDeliveredBtn(orderId) }} className="btn btn-warning btn-sm text-uppercase px-3"><b>Send To Delivered</b></button>
                                </div>
                                <div className="col-lg-6 col-md-6 col-12 text-lg-right text-md-right">
                                    <p><b className="mr-4">Total Price:</b><span style={{ fontSize: '1.1rem' }}>RS.{orderRequest[val].totalPrice}</span></p>
                                </div>
